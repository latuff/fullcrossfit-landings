/*! fullcrossfit-landing - v0.0.0 - 2014-06-29 */
...
// For Tracking
var getSectionLabel = function($el) {
    var $section = $el.closest('section[data-menu-label]');
    return $section.length ? (' section:' + $section.attr('data-menu-label').replace(/\s+/g, '-')) : '';
};
...
var Forms = {
    ...
    Newsletter: {
        onSubmit: function(e) {
            var $form = $(this);
            var val = $form.find('input[type="email"]').val();

            if ($.isEmail(val)) {
                ga('send', 'event', 'form', 'submit', 'newsletter form' + getSectionLabel($form));
            } else {
                e.preventDefault();
                $form.addClass('js-isInvalid');
                setTimeout(function() { $form.removeClass('js-isInvalid'); }, 0.8 * 1000);
            }
        }
    }
};
...
$(function() {
    ...
    if (! isTouch) {
        // init controller
        window.controller = new ScrollMagic();

        $menu = $('.Menu');
        var menu = [];
        $('.Box[id]').each(function() {
            var $el = $(this);
            menu.push('<a tabindex="-1" href="#' + $el.attr('id') + '"><span>' + $el.attr('data-menu-label') + '</span></a>');

            // highlight menu items when section is "in-view"
            var scene = new ScrollScene({triggerElement: this, duration: $(this).outerHeight(true)})
                .addTo(controller)
                .on('enter', function (e) {
                    $menu.find('a').removeClass('js-isActive');
                    $menu.find('a[href="#' + $el.attr('id') + '"]').addClass('js-isActive');
                });
        });
        $menu.html(menu.join(''));
    }
    ...
    var _scrollTo = function(e) {
        ...
        var toScroll = $elem.offset().top;
        var duration = Math.min(0.7, Math.max(0.2, Math.abs(currentScroll - toScroll) * 0.000222));

        if ($elem.length) {
            e.preventDefault();

            TweenMax.to(window, duration, {scrollTo: {y: toScroll}});
    ...
    $('.Menu').on('click', 'a[href^=#]', _scrollTo);
    $('.js-scrollTo').on('click', _scrollTo);

    $(".Video-wrap").fitVids();


    $('.ContactForm form').on('submit', Forms.Contact.onSubmit);
    $('.NewsletterForm form').on('submit', Forms.Newsletter.onSubmit);
